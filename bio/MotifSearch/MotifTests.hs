...
bruteforceMedianStringTest = TestCase (assertEqual "Finding 4-mer:" [k_mer] (bruteforceMedianString testData $ length k_mer))
        where k_mer = "AAAA"
              testData = evalRand (makeTestSeqs 5 20 k_mer) (mkStdGen 1)

branchAndBoundMedianStringTest = TestCase (assertEqual "Finding 4-mer:" k_mer (bbMedianString testData $ length k_mer))
        where k_mer = "AAAA"
              testData = evalRand (makeTestSeqs 5 20 k_mer) (mkStdGen 1)

branchAndBoundMedianStringTestAlt = TestCase (assertEqual "Finding 4-mer:" k_mer (preorderSearch testData $ length k_mer))
        where k_mer = "AACCT"
              testData = evalRand (makeTestSeqs 5 20 k_mer) (mkStdGen 1)


-- run tests with "runTestTT tests"
tests = TestList [bruteforceMedianStringTest, branchAndBoundMedianStringTest, branchAndBoundMedianStringTestAlt]
